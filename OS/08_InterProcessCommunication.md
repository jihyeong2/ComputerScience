...
> 각 프로세스는 독립적으로 실행된다. 즉, 다른 프로세스로부터 영향을 받지 않음을 의미한다. (스레드는 프로세스 내에서 자원을 공유하므로 영향을 받는다.)
>
> 하지만 프로세스 간의 통신을 해야 하는 상황이 있는데 이를 가능하게 해주는 것이 IPC 통신이다.
